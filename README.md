...
- **각자 본인 계정·본인 PC**로 에이전트를 쓰고, 로그에 **본인 이름표**를 단다(전원 참여 = ③ 채점 + Day2 1인 60초 미니시연으로 확인).
- 공유는 편한 방법으로:
  - **git** — 이 저장소를 fork/clone 해서 각자 작업 후 합치기 (커밋 이력은 ① 활용 가점 요소). git을 잘 모르면 안 써도 됨.
  - **공유 드라이브** — 킷 폴더를 사내 공유폴더에 두고 함께 편집.
  - **각자 따로 작업 후 합치기** — 개인별 `submit/PROCESS_LOG.md`를 각자 남긴 뒤, 제출 때 한 폴더로 합침(이름표로 구분).
- 최종 제출은 **한 폴더로 합쳐 1명이** 압축·제출.
