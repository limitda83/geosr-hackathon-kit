...
무엇을 하나 (큰 그림):
  1) SST 일자별 NC(1km, data/khoa_sst) 에서 28℃ 이상 '고수온' 격자를 본다.
  2) LSS 일자별 NC(250m, data/lss) 의 '저염분 플래그'를 1km(SST) 격자로 재가공한다.
...
def intersect_day(dt: date, sst_path: str, lss_path: str, out_dir: str,
                  ...
                  make_image: bool = True, write_csv: bool = True) -> dict:
    ...
    # --- 공통지역 격자목록 CSV (lat, lon, sst, lowsal_fraction) ---
    if write_csv:
        csv_dir = os.path.join(out_dir, "csv")
        os.makedirs(csv_dir, exist_ok=True)
        out_csv = os.path.join(csv_dir, base + ".csv")
        rr, cc = np.where(both)
        with open(out_csv, "w", encoding="utf-8-sig", newline="") as f:
            wcsv = csv.writer(f)
            wcsv.writerow(["date", "lat", "lon", "sst_celsius", "lss_class"])
            for r, c in zip(rr, cc):
                wcsv.writerow([dt.isoformat(), f"{tlat[r]:.4f}", f"{tlon[c]:.4f}",
                               f"{sst_arr[r, c]:.2f}", int(lss_class[r, c])])
...
def run(sst_dir: str, lss_dir: str, out_dir: str, start: date, end: date,
    ...
    print(f"저장: {os.path.abspath(out_dir)}  (NC + 지도PNG + CSV)\n")
    ...
    if rows:
        os.makedirs(out_dir, exist_ok=True)
        sumpath = os.path.join(out_dir, "summary_hotlowsal.csv")
        with open(sumpath, "w", encoding="utf-8-sig", newline="") as f:
            wcsv = csv.writer(f)
            wcsv.writerow(["date", "grid", "hot_count", "lowsal_count", "both_count",
                           "out_nc", "out_csv", "out_png", "error"])
            for r in rows:
                wcsv.writerow([r.get("date", ""), r.get("grid", ""),
                               r.get("hot_count", ""), r.get("lowsal_count", ""),
                               r.get("both_count", ""), r.get("out_nc", ""),
                               r.get("out_csv", ""), r.get("out_png", ""),
                               r.get("error", "")])
        print(f"\n요약 CSV: {sumpath}")
    return rows
...
def parse_args(argv=None):
    ...
    p.add_argument("--sst-dir", default="data/khoa_sst")
    p.add_argument("--lss-dir", default="data/lss")
    p.add_argument("--out", default="data/results/hotlowsal")
